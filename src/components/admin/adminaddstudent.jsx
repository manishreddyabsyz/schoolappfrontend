...
const Adminaddstudent = () => {
  ...
  const uploadStudents = async () => {
    ...
    const response = await axios.post("http://localhost:4000/add-students", fd);
  ...
  const getStudents = async () => {
    ...
    const response = await axios.get(`http://localhost:4000/get-students?&limit=5&page=${pageNo}`);
    console.log(response.data.data, "responsestudents");
    if (response.status === 200) {
      setDone(false);
  ...
  const deleteStudent= async (id) => {
    ...
    if (response.status === 200) {
      if(students.length===1) setPageCount(Math.abs(pageCount-1));
      setNotify({
        isOpen: true,
        message: response?.data?.message,
        type: "success",
  ...
  return (
    ...
    >
      ...
      {editOpenModal ? <Reuseablemodal id={id} editmodal={editOpenModal} setEditOpenModal={setEditOpenModal} role="STUDENT" data={students}  getData={getStudents}  setNotify={setNotify}/> : "" }
